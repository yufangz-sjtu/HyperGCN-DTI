...
def load_dataset(network_path, drug_protein_filename, train_filename=None, test_filename=None, 
    ...
    # 将数据集写入文件
    with open(network_path+"dti_index.txt", "w", encoding="utf-8") as f:
        for i in data_set:
            f.write(f"{i[0]}\t{i[1]}\t{i[2]}\n")

    # 创建 dtiedge.txt 文件
    with open(network_path+"dtiedge.txt", "w", encoding="utf-8") as f:
...
def load_dataset_from_name(dataName):
...
def load_feature(dataset_name,feature_list,normalize = False):
