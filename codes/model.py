...
class HyperGCNDTI(nn.Module):
    ...
    def SGC(self, feature, adj):
        ...
        adj = adj + (torch.eye(adj.shape[0]).to(device)) * 2
        deg = torch.sum(adj, dim=1)
        deg[deg <= 1e-10] = 1
        deg_inv = deg.pow(-0.5)
        deg_inv = deg_inv * torch.eye(adj.shape[0]).type(torch.FloatTensor).to(device)
        adj = torch.mm(deg_inv, adj)
        adj = torch.mm(adj, deg_inv).type(torch.FloatTensor)

        output = torch.mm(adj.to(device), feature.to(device))
    ...
    def _GCN(self, feature, adj, bias=None):
        ...
        # 归一化的邻接矩阵
        adj_norm = torch.mm(torch.mm(deg_inv_sqrt, adj), deg_inv_sqrt).type(torch.FloatTensor).to(device)
    ...
    def forward(self, node_num, drug_vec, protein_vec, protein_drug, drug_protein, HyGraph_Drug, HyGraph_protein, dateset_index, data, HyGraph_Structure_DPP, iftrain=True, d=None, p=None):
        ...
        HyGraph_ToGraph = torch.zeros((node_num[0] + node_num[1], node_num[0] + node_num[1])).to(device)
